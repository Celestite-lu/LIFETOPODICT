...
def _train(args):
    ...
    effective_nb_tasks = data_manager.nb_tasks
    max_tasks = args.get("max_tasks")
    if max_tasks is not None and int(max_tasks) > 0:
        effective_nb_tasks = min(data_manager.nb_tasks, int(max_tasks))
        logging.info(
            "Smoke task limit enabled: max_tasks={}, effective_nb_tasks={}, real_nb_tasks={}".format(
                max_tasks,
                effective_nb_tasks,
        ...
        logging.info(
            "Effective task count: {} (no max_tasks limit; real_nb_tasks={})".format(
                effective_nb_tasks,
                data_manager.nb_tasks,
            )
        )
    ...
    for task in range(start_task, effective_nb_tasks):
        if getattr(model, "_network", None) is None:
            logging.info("All params: 0 (backbone skipped; classifier-only feature-cache mode)")
            logging.info("Trainable params: 0 (backbone skipped; classifier-only feature-cache mode)")
        else:
        ...
        is_last_task = (task == effective_nb_tasks - 1)

# LifeTopoDict Fusion MVP Plan v2: Memory-Efficient + Auditable Topological Dictionary Prototypes

> 文档版本：v2.0（最终修订版）
> 生成日期：2026-05-21
> 基于 Agent A-J 全部发现 + 总指挥裁决 v2
> 修订记录：本版本根据总指挥裁决 1-10 对 v1.0 进行全面修订，每处修订标注 `[v2修订-裁决N]`

---

## 快速查阅卡 [v2修订-裁决7]

> 本卡为方案核心要素的一页纸摘要，供快速查阅。

**核心假设**：
HC-SOINN 的 class-local topology nodes 应保留节点身份，但节点中心可由共享字典 atoms 稀疏编码，从而获得跨类共享、可审计的拓扑生命周期管理，以及 memory-Pareto 优势。

**三个推进点的机制（一句话描述）**：
1. **Dictionary-Coded Nodes**：将 HC-SOINN 每类 ~18-21 个 raw 节点中心替换为全局共享字典的稀疏编码，实现跨类共享与存储压缩。
2. **Minimal Lifecycle**：为 atoms/nodes 附加三态状态机（plastic/protected/inactive），防止压缩后拓扑质量退化。
3. **Auditable Edges**：修复 edge age 持久化，引入 edge-aware local score 利用类内图结构增强推理。

**关键消融链**：
```
HC-SOINN (raw)
→ +dictionary-coding → +minimal lifecycle → +edge-aware scoring
（正向链）+ 逆向链（full → -edge → -lifecycle → -dict-coding）+ raw+lifecycle 消融 + Grid ablation (CUB-200)
```

**成功标准**：
| 编号 | 标准 | 量化阈值 |
|---|---|---|
| S1 | Accuracy 不显著退化 | drop ≤ 0.5pp **且统计显著**（paired t-test, p<0.05, 3 seeds） |
| S2 | Memory 显著降低 | compact memory 降低 ≥ 30%（不含 STAR 基线） |
| S5 | Lifecycle 有独立贡献 | worst-order accuracy 改善 ≥ 0.5pp **且 Old-New HM 变化 ≥ -0.3pp**，方向一致 |
| S6 | Edge 有实际使用 | edge_use_rate ≥ **5%** |
| S9 | 统计报告规范 | 所有 accuracy 指标报告均值 ± 标准差，配对检验 |

**Top-3 风险**：
1. HC-SOINN+STAR 已很强，融合方案难以超过 → P0 只承诺 memory-Pareto/非劣
2. 字典压缩损失局部拓扑质量 → 高残差触发 grow 或回退 raw node
3. PMI 少样本不可靠 → lifecycle 决策降级到 residual+usage+old support 驱动

**门控实验**：
- **Gate 1**：HC-SOINN vs NCM/SimpleCIL 增益 > 2pp（未通过则停止）
- **Gate 2**：residual < 0.3 的节点比例 > 80%
- **Gate 3**：compact memory < 50% HC-SOINN raw memory
...
[v2修订-裁决8] "Auditable" 维度明确化为两个可量化维度：
1. **结构透明性**：可以追溯每个节点由哪些 atoms 构成——给定节点 `v_{c,i}`，可以列出其 top-k atoms `a_{c,i}` 的非零索引及权重，解释节点的编码构成。
2. **状态可追溯性**：可以追溯每个 atom/节点的状态变迁历史——记录每次状态转换（plastic → protected → inactive → reactivated）及其触发条件（old_support、usage、residual），构成完整的生命周期审计轨迹。

来源：总指挥裁决 1；Agent F 建议的叙事切换（从 "lifecycle" 转为 "Memory-Efficient + Auditable Topology"）；Agent H 建议（"auditable" 需要可量化维度）；初步方案原文 L1-L16。
...
| **ICFL** [v2修订-裁决9] | Inter-Class Feature Learning | 也做跨类 feature sharing，通过类间特征迁移增强表示 | 区别：(1) ICFL 的共享发生在特征层，LifeTopoDict 的共享发生在字典 atom 层；(2) ICFL 无拓扑结构保留，LifeTopoDict 保留 HC-SOINN 的 class-local 图；(3) ICFL 无生命周期管理机制 |
...
1. **节点冗余**：每类 ~18-21 个独立子原型，无跨类共享机制，总存储随类数线性增长。来源：Agent C 论文提取；Agent E memory 估算。
...
[v2修订-裁决3] **20×10 协议下的字典初始化策略**：

在 20 tasks × 10 classes 协议下，字典初始化采用**延迟初始化**方案：

- **Base task 定义**：前 2 个 task（20 个类）。
- **第 1-2 个 task**：完全使用 raw HC-SOINN（不启用字典编码）。第 1 个 task 的 10 个类生成约 180-210 个节点中心（每类约 18-21 个节点）。
- **字典初始化时机**：第 2 个 task compress 完成后，收集前 2 个 task 所有节点中心（约 360-420 个），执行 K-Means 聚类，初始化字典 `D_0` [M, D]，M 从 {100, 200} 起始。
- **字典编码启用**：从第 3 个 task 开始启用字典编码。前 2 个 task 的节点中心也在此时刻执行首次稀疏编码。

选择延迟初始化（而非在第 1 个 task 后立即初始化）的原因：
1. 更简洁——不需要在第 1 个 task 后立即维护字典编码与 raw 节点的双轨逻辑
2. 360-420 个节点中心提供更充足的初始化样本量
3. 前 2 个 task 的 raw 节点行为作为 baseline 对照（与消融实验的 raw 变体一致）

来源：Agent I 裁决（延迟初始化更简洁）；Agent E 可行性审查。
...
[v2修订-裁决8] **审计追踪**：每次状态转换记录以下信息，实现状态可追溯性：
- 时间戳（task 编号）
- 转换方向（plastic → protected 等）
- 触发条件值（old_support、usage、residual 的具体数值）
- 影响范围（哪些类/节点受影响）
...
> 以下命题为字典编码的局部保距性提供理论保证。

**Proposition 1（Cosine-Ranking Preservation under Sparse Dictionary Coding）**：

设 v ∈ R^D 为归一化原始节点中心，`v_hat = norm(D @ a)` 为字典稀疏编码后的重构节点，其中 a 为 top-k 稀疏系数。定义重构残差 `r = 1 - cos(v, v_hat)`。

**命题陈述**：若重构残差 `r < ε`，则 `v_hat` 在 `v` 的 δ-邻域内保持 cosine-ranking 不变，即对于任意其他归一化向量 u：

```
若 cos(v, u) ≥ cos(v, u') + Δ（Δ > 0）
则 cos(v_hat, u) ≥ cos(v_hat, u') + Δ - 2δ
```

其中 `δ = O(√(ε))`，具体地 `δ ≤ √(2ε)`（利用 `1 - cos(v, v_hat) = r < ε` 和三角不等式）。

**直觉**：这是 Johnson-Lindenstrauss 类型的 bound。稀疏字典编码将高维节点中心投影到低维系数空间，如果重构误差足够小（残差 r 有界），则在原始空间中的余弦相似度排序在编码后近似保持。关键在于：(1) 残差 r 直接度量了编码的信息损失；(2) Gate 2 的 `residual < 0.3` 阈值对应 `δ ≤ √0.6 ≈ 0.77`，这足以保持粗粒度的 cosine-ranking；(3) 更严格的残差阈值（如 r < 0.1）可提供更紧的保距保证。

**局限**：此 bound 不保证 fine-grained ranking 的完全保持——当两个候选向量 u, u' 与 v 非常接近时（Δ 很小），δ 的扰动可能改变排序。这正是为什么 S1 要求 accuracy drop 有界（≤ 0.5pp）而非要求完全无损。

来源：Agent J Attack B 建议（Johnson-Lindenstrauss 类型 bound）；总指挥裁决 5。
...
| 序号 | 模块 | 修改内容 | 条件 |
|---|---|---|---|
| P1-1 | Edge-aware local score | 在 `predict_topk` 的 `final_scores` 中加入 edge 修正项 | edge age P0 修复完成 + edge_use_rate >= 5% [v2修订-裁决4] |
...
| 3 | +edge-aware scoring | 验证边是否真实影响推理 | edge_use_rate >= 5%, 至少一个稳定性指标改善 | edge_use_rate, edge_prediction_change, edge_ablation_delta |

### 4.2 [v2修订-裁决2] 补充消融变体

#### 4.2.1 raw+lifecycle 消融

**目的**：验证 lifecycle 是否有独立于字典编码的贡献。

```text
HC-SOINN (raw) → +minimal lifecycle（无字典编码）
```

在 raw HC-SOINN（无字典编码）上直接应用三态 lifecycle：
- Node-level protected/inactive 状态直接作用于 raw 节点
- Atom-level 状态不适用（无字典 atoms），用节点级 degree/usage 替代
- 预期：若此消融显示 lifecycle 对 raw HC-SOINN 也有改善，说明 lifecycle 机制本身有价值（独立于字典编码）；若无改善，说明 lifecycle 的价值主要体现在压缩后的质量保护。

#### 4.2.2 逆向消融链

**目的**：与正向链交叉验证，排除叠加顺序偏差。

```text
Full (dict+lifecycle+edge)
→ -edge
→ -lifecycle
→ -dict-coding
```

预期：逆向链的每步降级应与正向链的每步升级在数值上近似对称（差异 < 0.3pp）。

#### 4.2.3 Grid Ablation（限 CUB-200）

**目的**：穷举式验证各组件组合的独立/交互效应。

| 变体 | dict-coding | lifecycle | edge | 预期 |
|---|---|---|---|---|
| raw | - | - | - | baseline |
| dict-only | ✓ | - | - | 验证字典编码独立效果 |
| lifecycle-only [v2修订-裁决2] | - | ✓ | - | 验证 lifecycle 独立效果（与 4.2.1 一致） |
| dict+lifecycle | ✓ | ✓ | - | 验证字典+lifecycle 的交互效应 |
| dict+edge | ✓ | - | ✓ | 验证字典+edge 的交互效应（跳过 lifecycle） |
| full | ✓ | ✓ | ✓ | 完整系统 |

仅限 CUB-200 数据集（控制实验成本），3 seeds。

#### 4.2.4 实验运行数量估算 [v2修订-裁决7]

**P0 消融总运行数量**：

| 数据集 | 变体数 | seeds | 运行数 |
|---|---|---|---|
| CUB-200 | raw, +dict, +lifecycle, +edge, raw+lifecycle, full（正向链 4 + raw+lifecycle 1 + full 1）| 3 | 18 |
| CUB-200 Grid | 6 变体（含 raw 重复） | 3 | 18 |
| CUB-200 逆向链 | 4 变体 | 3 | 12 |
| ImageNet-R | raw, +dict, +lifecycle, +edge（正向链 4）| 3 | 12 |
| CIFAR-100 | raw, +dict（快速 sanity）| 3 | 6 |
| **小计** | | | **~66 运行** |

加上 Gate 实验（3 gates × 2 数据集 × 1-3 seeds ≈ 12 运行）和 baseline 重跑（NCM/SimpleCIL/HC-SOINN+STAR/ICFL ≈ 16 运行），P0 总计约 **94 个实验运行**。
...
[v2修订-裁决1] 补充说明：STAR 在源码 `simplecil_hc_soinn.py` 中未被调用（Agent D 发现），因此 STAR anchor 存储虽被原方案计入 memory 估算，但实际在当前系统中并未使用。论文 headline 数字基于不含 STAR 的口径（见附录 C 修订）。
...
| ICFL [v2修订-裁决9] | 跨类 feature sharing 对比方法 |
...
[v2修订-裁决3] **Base task 定义**：在 20×10 协议下，base = 前 2 个 task（20 个类）。前 2 个 task 使用 raw HC-SOINN，从第 3 个 task 开始启用字典编码。

[v2修订-裁决6] **Cal split 定义**：

采用第二种方案——用前 2 个 incremental task（task 3-4，即第 3、4 个 task，对应第 21-40 个类）作为 validation tasks。理由：更符合 CIL 场景的真实评估需求。

- Cal split 用于：超参数选择（M, k, λ₂, θ_support, θ_usage, T_inactive, γ, η 等）
- 选择策略：在 cal split 上评估各超参数组合的 Old-New HM，选择最优组合
- 选定后，在正式实验（task 5-20）中固定超参数，不再调优
- 正式实验报告的 accuracy 指标**仅统计 task 5-20 的结果**（排除 base task 和 cal split）

来源：Agent J Attack 3 建议（用 incremental task 更符合 CIL 场景）；总指挥裁决 6。

[v2修订-裁决6] **超参数清单表**：

| 超参数 | 符号 | 搜索范围 | 选择策略 | 选择 split |
|---|---|---|---|---|
| 字典规模 | M | {100, 200, 400} | 最小满足 Gate 2 的 M | Cal (task 3-4) |
| 稀疏度 | k | {3, 5, 8} | 在 Gate 2 M 下扫描，选 residual 最优 | Cal (task 3-4) |
| Ridge 正则 | λ₂ | {0.01, 0.1, 1.0} | 选 residual 最稳定（方差最小） | Cal (task 3-4) |
| Protected 阈值 | θ_support | {0.3, 0.5, 0.7} | 选 Old-New HM 最优 | Cal (task 3-4) |
| Inactive usage 阈值 | θ_usage | {0.01, 0.05, 0.1} | 选 stale-risk precision 最优 | Cal (task 3-4) |
| Inactive 连续 task 数 | T_inactive | {2, 3, 5} | 选 worst-order 最优 | Cal (task 3-4) |
| Inactive residual 阈值 | θ_r_inactive | {0.3, 0.5, 0.7} | 选 Old-New HM 最优 | Cal (task 3-4) |
| Edge 邻居权重 | γ | {0.0, 0.1, 0.3, 0.5} | 选 edge_use_rate ≥ 5% 且 HM 不降的最大 γ | Cal (task 3-4) |
| Edge age 权重 | η | {0.0, 0.01, 0.05, 0.1} | 选 edge ablation delta 最优 | Cal (task 3-4) |
| K-Means 初始化种子 | — | 固定 42 | 固定 | — |

总超参数组合数：3 × 3 × 3 × 3 × 3 × 3 × 3 × 4 × 4 = 26,244（全网格不可行）。采用**分步选择**：
1. 先选 (M, k, λ₂)（Gate 2 驱动）
2. 再选 (θ_support, θ_usage, T_inactive, θ_r_inactive)（lifecycle 阶段）
3. 最后选 (γ, η)（edge 阶段）
实际搜索组合数降低到约 27 + 81 + 16 = 124 组合。

来源：Agent J Attack 3；总指挥裁决 6。
...
| **ICFL** [v2修订-裁决9] | Inter-Class Feature Learning：跨类 feature sharing 对比方法。ICFL 也做跨类特征迁移，但其 sharing 发生在 feature 层面（通过类间特征蒸馏/迁移），而 LifeTopoDict 的 sharing 发生在字典 atom 层面。ICFL 无拓扑结构保留、无生命周期管理、无 class-local graph。需注意 ICFL 可能在某些 backbone 下已包含在 SimpleCIL 的变体中，需确认不重复。 |
...
| ICFL [v2修订-裁决9] | 完整验证阶段也需包含 ICFL 对比 |

来源：Agent D 源码分析（`simplecil_hc_soinn.py` 为当前集成方式）；Phase3A A17；Agent J Attack C（添加 ICFL baseline）。
...
| 指标 | 说明 |
|---|---|
| Average Accuracy | 全任务平均准确率（报告均值 ± 标准差）[v2修订-裁决4] |
| Final Accuracy | 最终任务准确率（报告均值 ± 标准差）[v2修订-裁决4] |
| Forgetting | 标准遗忘率（报告均值 ± 标准差）[v2修订-裁决4] |
| Old/New Acc | 旧类/新类分别准确率（报告均值 ± 标准差）[v2修订-裁决4] |
| Old-New HM | 旧类新类调和平均（主指标之一，暴露 old-new 平衡）（报告均值 ± 标准差）[v2修订-裁决4] |
...
| worst-order accuracy | lifecycle | 最差任务顺序下的准确率（报告均值 ± 标准差）[v2修订-裁决4] |
...
[v2修订-裁决4] **新增 S9 统计报告规范**：所有 accuracy 指标的差异报告均值 ± 标准差（跨 3 seeds），并进行配对检验（paired t-test，p < 0.05）。具体格式：
- 表格中每个 accuracy 值报告为 `mean ± std`
- 差异报告为 `Δ = X.XX ± X.XX (p = X.XXX)`
- 方向一致性：3 个 seeds 中的符号方向必须一致（即不允许 2 正 1 负声称"改善"）

来源：初步方案 L259-290；Phase3A A1/A7/A8/A11（指标设计原则）；总指挥裁决 4。
...
| R5 | 字典初始化质量决定 base 成败 | 高 | 高 | 使用 K-Means/class-means 初始化而非随机初始化；延迟初始化（前 2 task raw）增加初始化样本量；base task 后验证 residual 分布；Gate 2 检查 [v2修订-裁决3] | Phase3A X3/B13/B14 |
...
| R8 | Edge 名义使用（对推理无实际贡献） | 中高 | 中 | 必报 edge-use、prediction change、edge ablation；无贡献则降 appendix；S6 阈值提高到 5% [v2修订-裁决4] | Agent F |
...
| R12 | 机制堆叠审稿风险（三个推进点叠加） | 高 | 高 | 每模块绑定不同证据维度，不只看 Avg Acc；消融链逐层叠加 + 逆向消融 + Grid ablation [v2修订-裁决2] |

### 6.3 [v2修订-裁决10] 灰色地带处理规则

在 P0 验证完成后，可能出现以下灰色地带，需按规则处理：

| 灰色地带 | 条件 | 处理 |
|---|---|---|
| **仅字典编码有效** | 只有 dictionary-coding 有贡献，lifecycle/edge 无独立贡献，但 memory-Pareto 优势 ≥ 40% | 降为 **"efficient compression"** 论文。论文标题和叙事围绕"字典压缩 HC-SOINN 拓扑"，不再声称 lifecycle/edge 贡献。lifecycle/edge 降为 appendix/future work。 |
| **中等 accuracy 代价但显著 memory 优势** | accuracy drop 在 0.5-1.0pp 但 memory 降幅 ≥ 50% | 报告为 **trade-off**，不声称"非劣"（no-worse-than）。论文叙事改为"可控精度代价下的显著存储优势"，提供 memory-accuracy trade-off 曲线。 |
| **仅 CUB-200 成功** | 仅 CUB-200（细粒度）成功但 ImageNet-R（域偏移）失败 | **限制 claim 范围为细粒度场景**。论文标题/abstract 明确标注 "on fine-grained benchmarks"，ImageNet-R 结果报告为 limitation。分析失败原因（可能是域偏移下字典编码的 residual 过高）。 |
| **lifecycle 有微小负面效应** | lifecycle 使 Old-New HM 下降 0.1-0.3pp 但 memory 优势显著 | 不声称 lifecycle "改善"稳定性，改为声称 lifecycle "maintains" 稳定性同时提供结构化管理能力。 |

来源：Agent I 裁决；Agent J 审查。
...
4. edge-use < 5% [v2修订-裁决4]，且 edge ablation 对 margin/HM/worst-order 无稳定正效应
...
> v1 原始时间线 4-6 周过于乐观，修订为 8-10 周，预留 2 周缓冲用于 Gate 2/3 重试。

### 7.1 最简可行性验证：约 6-7 周

| 阶段 | 时间 | 内容 | 交付物 |
|---|---|---|---|
| 协议与源码对齐 | 1 周 | split 固定；compress 时序确认；memory ledger 设计；edge age 持久化修复 (P0-2)；base task 定义确认（前 2 task raw）[v2修订-裁决3] | 可复现的 baseline pipeline |
| Gate 1 验证 | 0.5 周 | CUB-200 单 seed 跑 HC-SOINN raw vs NCM/SimpleCIL | Gate 1 通过/不通过决定 |
| Dictionary-coded nodes (P0-1) | 1-1.5 周 | 实现 D、a、v_hat；延迟初始化逻辑；跑 CUB 单 seed | Gate 2/3 结果 |
| **Gate 2/3 重试缓冲** | **0.5-1 周** | 若 Gate 2/3 未通过，调整 M/k 后重试 | 调整后的 Gate 2/3 结果 |
| Minimal lifecycle (P0-3) | 1 周 | 三态实现；protected deletion gate；inactive readout mask；状态审计追踪 [v2修订-裁决8] | CUB lifecycle 消融 |
| CUB 3 seeds + 新增消融变体 | 1.5-2 周 | 完整 P0 消融链（正向 + raw+lifecycle + 逆向 + Grid）；画 memory-Pareto [v2修订-裁决2] | 决定是否进入 P1 |
| ImageNet-R 初验 + ICFL baseline | 1 周 | ImageNet-R P0 消融；ICFL baseline 对比 [v2修订-裁决9] | 双数据集验证结果 |

**关键决策点**：Gate 1 未通过 → 停止。Gate 2/3 未通过 → 调整 M/k 后重试一次（已预留缓冲时间）。CUB 3 seeds P0 消融不满足成功标准 → 进入灰色地带评估（6.3 节）或停止。

### 7.2 完整融合验证：约 3-4 周

| 阶段 | 时间 | 内容 |
|---|---|---|
| Edge-aware scoring (P1-1) | 1 周 | 实现 edge-aware local score；CUB+INR 消融 |
| Feature-adapted STAR head-to-head (P1-3/4) | 1-2 周 | STAR-gated 集成；drift 作用于 coefficient |
| 完整矩阵 + sensitivity + failure cases | 1 周 | 全消融矩阵；阈值敏感性；per-dataset failure 分析；ICFL 完整对比 |

### 7.3 实验运行数量与资源估算 [v2修订-裁决7]

| 阶段 | 运行数 | GPU 时间估算（单运行约 30min）|
|---|---|---|
| Gate 实验 | ~12 | 6 小时 |
| P0 正向消融链 | ~30 | 15 小时 |
| P0 新增消融（raw+lifecycle + 逆向 + Grid） | ~36 | 18 小时 |
| Baseline 重跑（含 ICFL） | ~20 | 10 小时 |
| P1 edge + STAR | ~30 | 15 小时 |
| Sensitivity + failure | ~20 | 10 小时 |
| **总计** | **~148 运行** | **~74 GPU 小时** |

总时间线：**8-10 周**（含 2 周缓冲）。

来源：Agent E（代码改动范围约 100 行）；Agent I 裁决（时间线修正为 8-10 周）；初步方案 L335-353。
...
| 编号 | 标准 | 量化阈值 | 统计要求 [v2修订-裁决4] |
|---|---|---|---|
| S1 | Accuracy 不显著退化 | CUB-200 或 ImageNet-R 至少一个数据集上，accuracy drop ≤ 0.5pp | **且统计显著**（paired t-test, p < 0.05, 3 seeds） |
| S2 | Memory 显著降低 | compact deployable memory 降低 ≥ 30% | 基于不含 STAR anchor 的口径 [v2修订-裁决1] |
...
| S5 | Lifecycle 有独立贡献 | worst-order accuracy 改善 ≥ 0.5pp **且 Old-New HM 变化 ≥ -0.3pp** [v2修订-裁决4] | **跨 3 seeds 方向一致** |

### 8.2 Edge 进入主文的附加条件

| 编号 | 标准 | 量化阈值 |
|---|---|---|
| S6 | Edge 有实际使用 | edge_use_rate ≥ **5%**（从 1% 提高到 5%）[v2修订-裁决4] |
...
| S9 | [v2修订-裁决4] 统计报告规范 | 所有 accuracy 指标报告均值 ± 标准差，配对检验（paired t-test, p < 0.05），跨 3 seeds 方向一致 |

### 8.4 失败判定

若最终只能写成 "HC-SOINN with dictionary bottleneck"，且 lifecycle/edge 没有独立诊断贡献，应停止把 LifeTopoDict 作为主线。

### 8.5 [v2修订-裁决10] 灰色地带处理规则

详见 6.3 节。核心原则：
- **不掩盖部分失败**：如实报告每个组件的独立贡献
- **调整 claim 范围**：限制为细粒度场景/efficient compression/trade-off
- **不声称"非劣"除非满足 S1 的统计显著性**
...
| STAR 在 `simplecil_hc_soinn.py` 中没被调用 | `models/simplecil_hc_soinn.py` 全文 | 验证了 P0 不启用 STAR 的决策；也说明 STAR anchor 存储不应计入 baseline memory |
...
## 附录 C：字典规模 M 预算分析 [v2修订-裁决1]

> v1 原始 memory 估算存在重大问题：将 STAR anchor 存储（~36MB）计入 HC-SOINN raw baseline，但 STAR 在源码中未被调用，导致压缩比虚高（91.8%）。本附录全面修正。

### C.1 Memory 估算（修正版）

**口径 (a)：不含 STAR anchor（P0 MVP 对比，论文 headline 口径）**

[v2修订-裁决1] 此口径排除 STAR anchor 存储，因为：
1. STAR 在源码 `simplecil_hc_soinn.py` 中未被调用（Agent D 发现）
2. P0 MVP 不启用 STAR
3. 论文 headline 数字必须基于实际使用的组件

| 组件 | HC-SOINN Raw（不含 STAR） | LifeTopoDict (M=200) | 说明 |
|---|---|---|---|
| 节点 center_raw (200 类 x 20 节点) | 200 x 20 x 768 x 4B = **11.7 MB** | — | LifeTopoDict 不再存储原始 center |
| 节点 center (normalized) | — | 200 x 20 x 768 x 4B = **11.7 MB** | LifeTopoDict 存储 v_hat（materialized）|
| 字典 D | — | 200 x 768 x 4B = **0.6 MB** | 新增 |
| 稀疏系数 a (k=5) | — | 200 x 20 x 5 x 4B = **0.08 MB** | 新增（实际存储 sparse index + value） |
| 节点 metadata（count, state, residual） | — | 200 x 20 x (4+1+4)B ≈ **0.04 MB** | 新增 |
| Atom state | — | 200 x 1B ≈ **0.001 MB** | 新增 |
| Edge adjacency + age | 200 x 20 x 20 x (4+4)B ≈ **2.5 MB** | 200 x 20 x 20 x (4+4)B ≈ **2.5 MB** | 保留（age 持久化修复后） |
| **总计（不含 STAR）** | **~23.4 MB** [v2修订-裁决1] | **~12.5 MB** [v2修订-裁决1] | — |

**压缩比（不含 STAR）**：**(23.4 - 12.5) / 23.4 ≈ 47%** [v2修订-裁决1]

**口径 (b)：含 STAR anchor（完整系统对比）**

| 组件 | HC-SOINN Raw + STAR | LifeTopoDict | 说明 |
|---|---|---|---|
| 以上所有（不含 STAR） | 23.4 MB | 12.5 MB | 同口径 (a) |
| STAR image anchors | 200 x 20 x 224x224x3 ≈ **36 MB** | — | P0 不启用 |
| **总计（含 STAR）** | **~59.4 MB** [v2修订-裁决1] | **~12.5 MB** [v2修订-裁决1] | — |

**压缩比（含 STAR）**：**(59.4 - 12.5) / 59.4 ≈ 79%** [v2修订-裁决1]

### C.2 论文 headline 数字使用规范 [v2修订-裁决1]

- **论文 headline、abstract、introduction**：使用口径 (a) 数字（不含 STAR），报告约 47% 压缩
- **论文正文 memory 分析小节**：同时报告口径 (a) 和口径 (b)，明确解释两种口径的差异
- **成功标准 S2 的 30% 降幅**：基于口径 (a) 的 23.4MB 基线计算
...
来源：Phase3A X9（字典容量与任务序列长度的关系）；Phase3A A16/F1；Agent I P0-CRITICAL（memory 估算修正）。

---

*文档生成日期：2026-05-21*
*文档版本：v2.0（最终修订版）*
*基于 Agent A-J 全部发现 + 总指挥裁决 v2（裁决 1-10）*
*所有设计决策可追溯至具体来源标注*
*每处修订标注 `[v2修订-裁决N]`*

...
class Learner(BaseLearner):
    ...
    def __init__(self, args):
        ...
        if self._feature_cache_classifier_only:
            ...
            logging.info(
                ...
                "initialization for raw HC-SOINN; classifier_device=%s",
                self._classifier_device,
            )
        else:
    ...
    def _log_hc_soinn_memory(self):
        if not hasattr(self.hc_soinn, "_compute_memory_breakdown"):
            return
        try:
            memory = self.hc_soinn._compute_memory_breakdown()
            logging.info(
                "[HC-SOINN] Memory: compact=%.4f MB, actual=%.4f MB, "
                "node_centers=%.4f MB, edges=%.4f MB, edge_rel=%.4f MB, "
                "caches=%.4f MB, buffers=%.4f MB, frozen=%.4f MB",
                memory.get("compact_deployable_mb", 0.0),
                memory.get("actual_implementation_mb", 0.0),
                memory.get("node_centers_mb", 0.0),
                memory.get("edges_mb", 0.0),
                memory.get("edge_reliability_mb", 0.0),
                memory.get("caches_mb", 0.0),
                memory.get("buffers_mb", 0.0),
                memory.get("frozen_mb", 0.0),
            )
        except Exception as e:
            logging.warning(f"HC-SOINN memory breakdown unavailable: {e}", exc_info=True)
    ...
    def after_task(self):
        ...
        self._log_hc_soinn_memory()
        
        self._known_classes = self._total_classes
        self._hc_soinn_compressed_for_task = False
        
        if self.args.get("visualize_tsne", False):
            try:
        ...
        with torch.no_grad():
            for _, (_, inputs, targets) in enumerate(loader):
                ...
                topk_pred = self.hc_soinn.predict_topk(
                    features, self.topk, self._total_classes, device=self._classifier_device
                )
    ...
    def _visualize_hc_soinn_tsne(self, target_classes=None):
        ...
        try:
            train_dataset = get_cached_feature_dataset(
                self.args,
                self.data_manager,
            ...
            log_feature_cache_loader(train_loader, "tsne_train_loader")
            
            sample_features = []
            sample_labels = []
            
            from_cache = is_cached_feature_loader(train_loader)
            if not from_cache and self._network is None:
                raise RuntimeError(
                    "Backbone is skipped, but t-SNE train_loader is not backed by cached features."
                )
            if self._network is not None:
                self._network.eval()
            with torch.no_grad():
